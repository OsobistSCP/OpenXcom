...
{
	int bonus = 100;
	if (_game != nullptr && _game->getSavedGame() != nullptr)
	{
		int loyalty = _game->getSavedGame()->getLoyalty();
...
/*bool MasterMind::updateReputationLvl(DiplomacyFaction *faction, bool initial)
{
	...
	if (repLevels)
	{
		...
		if (repName == "STR_ALLY")
			newLvl = 6;
		else if (repName == "STR_HONORED")
			newLvl = 5;
		else if (repName == "STR_FRIENDLY")
			newLvl = 4;
		else if (repName == "STR_UNFRIENDLY")
			newLvl = 2;
		else if (repName == "STR_HOSTILE")
			newLvl = 1;
		else if (repName == "STR_HATED")
			newLvl = 0;
